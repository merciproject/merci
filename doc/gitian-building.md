...
*Setup instructions for a Gitian build of Merci Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Merci
...
- [Building Merci Core](#building-merci-core)
...
Clone the git repositories for merci and Gitian.
...
```bash
sudo cp /home/debian/merci/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the merci repository under 'Fetch and create inputs' to install sources which require
...
To build Merci Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the merci repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/merci/.git/
...
For example:
```bash
URL=https://github.com/laanwj/merci.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit merci=${COMMIT} --url merci=${URL} ../merci/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit merci=${COMMIT} --url merci=${URL} ../merci/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit merci=${COMMIT} --url merci=${URL} ../merci/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-merci:
./bin/gbuild --commit merci=${COMMIT},cpp-eth-merci=${ETHCOMMIT} --url merci=${URL},cpp-eth-merci=${ETHURL} ../merci/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the merci git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../merci/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/merci
SIGPATH=/some/root/path/merci-detached-sigs

./bin/gbuild --url merci=${BTCPATH},signature=${SIGPATH} ../merci/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/merci-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/merci-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/merci-osx-build.assert
```
...
[merciproject/gitian.sigs](https://github.com/merciproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@mercibq.com) and he will commit them.
