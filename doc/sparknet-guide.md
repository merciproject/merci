...
# Using Smart Contracts with Merci

The smart contract interface in Merci still requires some technical knowledge. The GUI is not completed yet, so all smart contract interation must happen either using `merci-cli` at the command line, or in the debug window of `merci-qt`. 
...
(note, if using the debug window in the Merci Qt application, don't include `./merci-cli` in the commands)

First, we need to deploy the contract:

    ./merci-cli createcontract  300000
...
Now wait for your contract to be included in a block. You should be able to confirm it made it into a block by using:
    ...
    ./merci-cli getaccountinfo $CONTRACT
...
Now, because we are not changing state, we use `callcontract`:
    ...
    ./merci-cli callcontract $CONTRACT 3450bd6a
...
Note we use --lenient so that we do not need to provide a full 256bit value as a parameter. Now, we can execute the contract directly:
    ...
    ./merci-cli sendtocontract $CONTRACT 3fb5c1cb000000000000000000000000000000000000000000000000000000000001e240
...
This will send 10 tokens to the contract:
    ...
    ./merci-cli sendtocontract $CONTRACT d0e30db0 10

And then, to withdraw them it's also very simple:

    ./merci-cli sendtocontract $CONTRACT 3ccfd60b
...
If you want to control the exact address that the contract sends coins to, you can also explicitly specify the sender. Note that you must be capable of sending coins from that address (you can't use someone elses address). One of my wallet addresses is qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt, so I'll use that:
    ...
    ./merci-cli sendtocontract $CONTRACT 3ccfd60b 0 190000 0.0000001 qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt
...
Note that if you get the error "Sender address does not have any unspent outputs", then you should send some coins to that address (they must be spent in order to prove that you own that address). This can be accomplished with any amount of coins:
    ...
    ./merci-cli sendtoaddress qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt 0.001
...
* Q: "I think I found a bug in Merci" A: Please report any bugs at https://github.com/merciproject/merci/issues



# New Merci RPC Commands

Merci supports all of the RPC commands supported by Bitcoin Core, but also includes the following commands unique to Merci:

* `createcontract` - This will create and deploy a new smart contract to the Merci blockchain. This requires gas.
* `callcontract` - This will interact with an already deployed smart contract on the Merci blockchain, with all computation taking place off-chain and no persistence to the blockchain. This does not require gas
* `sendtocontract` - This will interact with an already deployed smart contract on the Merci blockchain. All computation takes place on-chain and any state changes will be persisted to the blockchain. This allows tokens to be sent to a smart contract. This requires gas.
...
# New Merci Command Line Arguments

Merci supports all of the usual command line arguments that Bitcoin Core supports. In addition it adds the following new command line arguments:

* `-record-log-opcodes` - This will create a new log file in the Merci data directory (usually ~/.merci) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 

# Untested features

Some features included in Bitcoin Core have not been tested in it's porting to Merci. This includes:

* Pruning

# EVM Smart Contract Changes and Limitations

Because of Merci's underlying technical differences, there are a few operations that can have different results or limitations when executed in Merci than when compared to Ethereum. 

These include the following, though there may be others introduced in the future:  

* The gas schedule for Merci is different from Ethereum. Certain operations are more or less expensive. As such, gas cost estimators designed for Ethereum will not give accurate results for Merci. We will develop our own gas estimating tools as well as fully documenting these differences at a later date. 
...
Additional documents for the overall design and expected results of various operations is available at the ITD repository here: https://github.com/merciproject/merci-itds
