...
class MerciManyValueRefundsFromSameTxTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx = CTransaction()
        tx.vin = [make_vin(self.node, int(2*(COIN + MERCI_MIN_GAS_PRICE*100000)))]
        tx.vout = []
        tx.vout.append(CTxOut(int(COIN), CScript([b"\x04", CScriptNum(100000), CScriptNum(MERCI_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])))
        tx.vout.append(CTxOut(int(COIN), CScript([b"\x04", CScriptNum(100000), CScriptNum(MERCI_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])))
...
if __name__ == '__main__':
    MerciManyValueRefundsFromSameTxTest().main()
