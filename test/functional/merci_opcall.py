...
from test_framework.merci import *
from test_framework.merciconfig import *
import sys
...
class OpCallTest(BitcoinTestFramework):
    ...
    def send_multiple_op_call_txs_with_counter_check(self, num_txs, outputs, counter_should_increase_by):
        ...
        while i < num_txs and len(unspents) > 0:
            # Select as input a tx which has at least 5 merci spendable
            for tx_i in range(len(unspents)):
                print(int(unspents[tx_i]['amount']*COIN), 1000000*MERCI_MIN_GAS_PRICE, unspents[tx_i]['spendable'])
                if int(unspents[tx_i]['amount']*COIN) == 1000000*MERCI_MIN_GAS_PRICE and unspents[tx_i]['spendable']:
                    print('FOUND')
                    break
            else:
                assert(False)
    ...
    def many_calls_in_same_tx_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, counter_should_increase_by=2, input_value=2*1000000*MERCI_MIN_GAS_PRICE)

    # Sends a normal raw op_call tx with a single output.
    def normal_op_call_output_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\x7f", CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, counter_should_increase_by=1, input_value=0x7fff*MERCI_MIN_GAS_PRICE)

    # Sends a tx containing 1 op_call output where txfee == gas_price*gas_limit.
    def gas_equal_to_tx_fee_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, counter_should_increase_by=1, input_value=1000000*MERCI_MIN_GAS_PRICE)

    # Sends a tx containing 1 op_call output where txfee < gas_price*gas_limit.
    def gas_exceeding_tx_fee_100001_1_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(10000001), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=1000001*MERCI_MIN_GAS_PRICE-1, should_throw=True)

    # Sends a tx containing 1 op_call output where txfee < gas_price*gas_limit.
    def gas_exceeding_tx_fee_100001_2_test(self):
        outputs = []
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000001), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=1000000*MERCI_MIN_GAS_PRICE, should_throw=True)
    ...
    def two_calls_in_same_tx_exceeding_tx_fee_test(self):
        ...
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, input_value=2000000*MERCI_MIN_GAS_PRICE-1, should_throw=True)
    ...
    def gas_limit_signedness_test(self):
        ...
        while len(gas_limit) < 20:
            outputs.append(make_op_call_output(0, b"\x04", gas_limit, CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
            self.send_one_op_call_tx_with_counter_check(outputs, should_throw=True, input_value=min(max(int(bytes_to_hex_str(gas_limit), 16)*MERCI_MIN_GAS_PRICE, 10000000), 1000000000))
            gas_limit += b"\xff"
    ...
    def gas_limit_signedness_one_valid_test(self):
        outputs = []
        gas_limit = b"\xff"
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\xff\x00", CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        outputs.append(make_op_call_output(0, b"\x04", b"\xff\xff", CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("371303c0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, should_throw=True, input_value=2*0xffff*MERCI_MIN_GAS_PRICE)
    ...
    def send_tx_with_value_test(self):
        outputs = []
        # d0e30db0 deposit()
        outputs.append(make_op_call_output(100000000, b"\x04", CScriptNum(1000000), CScriptNum(MERCI_MIN_GAS_PRICE), bytes.fromhex("d0e30db0"), bytes.fromhex(self.contract_address)))
        self.send_one_op_call_tx_with_counter_check(outputs, counter_should_increase_by=0, input_value=100000000+1000000*MERCI_MIN_GAS_PRICE)
    ...
    def run_test(self):
        ...
        self.node.sendmany("", {self.node.getnewaddress(): 1000000*MERCI_MIN_GAS_PRICE / Decimal('100000000') for i in range(200)})
