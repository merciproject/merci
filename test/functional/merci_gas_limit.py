...
class MerciGasLimit(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx.vout = [CTxOut(0, CScript([b"\x04", int(5*COIN), MERCI_MIN_GAS_PRICE, b"\x00", bytes.fromhex(contract_address), OP_CALL])) for i in range(NUM_OUTPUTS)]
        tx.rehash()
