...
class SendToContractTest(BitcoinTestFramework):
    ...
    def sendtocontract_verify_storage_and_balance_test(self):
        # call set owner with 9999999999999999999999999999999999999999 as owner with 100 merci
    ...
    def sendtocontract_specify_sender_test(self):
        ...
        self.node.generate(1)
        # call setSenderAsOwner with 100 merci
        ret = self.node.sendtocontract(self.contract_address, "2bcf51b4", 100, 1000000, MERCI_MIN_GAS_PRICE_STR, "qabmqZk3re5b9UpUcznxDkCnCsnKdmPktT")
        assert('txid' in ret)
        assert('sender' in ret)
        assert('hash160' in ret)
        time.sleep(0.1)
    ...
    def sendtocontract_no_broadcast(self):
        ...
        self.node.generate(1)
        # call setSenderAsOwner with 100 merci
        ret = self.node.sendtocontract(self.contract_address, "2bcf51b4", 100, 1000000, MERCI_MIN_GAS_PRICE_STR, "qabmqZk3re5b9UpUcznxDkCnCsnKdmPktT", False)
