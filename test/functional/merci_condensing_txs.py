...
class CondensingTxsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        T4_raw = make_transaction(self.node, [make_vin(self.node, 3*COIN)], [make_op_call_output(2*COIN, b"\x04", 22000, CScriptNum(MERCI_MIN_GAS_PRICE), hex_str_to_bytes(self.share_abi), hex_str_to_bytes(self.sender2))])
        T4_id = self.node.sendrawtransaction(T4_raw)
        T5_id = self.node.sendtocontract(self.sender2, self.withdrawAll_abi, 0, 1000000, MERCI_MIN_GAS_PRICE_STR, A1)['txid']
