...
#include <merci/mercitransaction.h>
...
class MerciState : public dev::eth::State {
    
public:

    MerciState();

    MerciState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, MerciTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
private:
    ...
    dev::Address createMerciAddress(dev::h256 hashTx, uint32_t voutNumber);
...
struct TemporaryState{
    std::unique_ptr<MerciState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<MerciState>& _globalStateRef) :
...
public:
    ...
    CondensingTX(MerciState* _state, const std::vector<TransferInfo>& _transfers, const MerciTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const MerciTransaction& transaction;
