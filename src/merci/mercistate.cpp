...
#include <merci/mercistate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

MerciState::MerciState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = MerciState::openDB(_path + "/merciDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

MerciState::MerciState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute MerciState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, MerciTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createMerciAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            merci::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> MerciState::vins() const // temp
{
...
void MerciState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* MerciState::vin(dev::Address const& _a) const
{
    return const_cast<MerciState*>(this)->vin(_a);
}

Vin* MerciState::vin(dev::Address const& _addr)
{
...
// void MerciState::commit(CommitBehaviour _commitBehaviour)
// {
...
void MerciState::kill(dev::Address _addr)
{
...
void MerciState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address MerciState::createMerciAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void MerciState::deleteAccounts(std::set<dev::Address>& addrs){
...
void MerciState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void MerciState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
