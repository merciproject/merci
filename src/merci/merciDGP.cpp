#include <merci/merciDGP.h>

void MerciDGP::initDataEIP158(){
...
bool MerciDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule MerciDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t MerciDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t MerciDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t MerciDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t MerciDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool MerciDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void MerciDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void MerciDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void MerciDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void MerciDGP::createParamsInstance(){
...
dev::Address MerciDGP::getAddressForBlock(unsigned int blockHeight){
...
void MerciDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MerciDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void MerciDGP::parseStorageOneUint64(uint64_t& value){
...
void MerciDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule MerciDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
