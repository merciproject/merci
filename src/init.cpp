...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/merciproject/merci>";
    const std::string URL_WEBSITE = "<https://mercibq.com>";
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                fs::path merciStateDir = GetDataDir() / "stateMerci";
                bool fStatus = fs::exists(merciStateDir);
                const std::string dirMerci(merciStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsMercistate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<MerciState>(new MerciState(dev::u256(0), MerciState::openDB(dirMerci, hashDB, dev::WithExisting::Trust), dirMerci, existsMercistate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::merciMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(merciStateDir.string()));
                if (fReset) {
