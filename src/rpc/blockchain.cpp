...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // merci
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // merci
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // merci
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // merci
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination merciSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(merciSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&merciSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of merci addresses\n"
