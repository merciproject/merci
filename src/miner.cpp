...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setMerciSchedule(merciDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = merciDGP.getBlockSize(nHeight);
    minGasPrice = merciDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = merciDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    MerciTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractMerciTX resultConverter;
    if(!convert.extractionMerciTransactions(resultConverter)){
    ...
    for(MerciTransaction merciTransaction : merciTransactions){
        txGas += merciTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + merciTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(merciTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, merciTransactions, hardBlockGasLimit);
...
void StakeMercis(bool fStake, CWallet *pwallet)
{
