...
#include <merci/storageresults.h>


extern std::unique_ptr<MerciState> globalState;
...
using ExtractMerciTX = std::pair<std::vector<MerciTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // merci
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashMerciRoot,
    const dev::h256& oldHashStateRoot, const std::vector<MerciTransaction>& transactions);
...
class MerciTxConverter{

public:

    MerciTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionMerciTransactions(ExtractMerciTX& merciTx);
...
private:
    ...
    MerciTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<MerciTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
