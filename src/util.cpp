...
const char * const BITCOIN_CONF_FILENAME = "merci.conf";
const char * const BITCOIN_PID_FILENAME = "mercid.pid";
...
bool OpenDebugLog()
{
    ...
    assert(fileoutVM == nullptr); // merci
    assert(vMsgsBeforeOpenLog);
    fs::path pathDebug = GetDebugLogPath();
    fs::path pathDebugVM = GetDebugVMLogPath(); // merci
    fileout = fsbridge::fopen(pathDebug, "a");
    fileoutVM = fsbridge::fopen(pathDebugVM, "a"); // merci
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Merci
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Merci
    // Mac: ~/Library/Application Support/Merci
    // Unix: ~/.merci
#ifdef WIN32
    // Windows
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Merci Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Merci Core developers";
    }
