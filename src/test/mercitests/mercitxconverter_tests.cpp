...
void checkResult(bool isCreation, std::vector<MerciTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    MerciTxConverter converter(transaction, NULL);
    ExtractMerciTX merciTx;
    BOOST_CHECK(converter.extractionMerciTransactions(merciTx));
    std::vector<MerciTransaction> result = merciTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(mercitxconverter_tests, TestingSetup)
