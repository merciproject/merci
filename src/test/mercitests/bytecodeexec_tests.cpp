...
#include <mercitests/test_utils.h>
...
BOOST_AUTO_TEST_CASE(bytecodeexec_txs_empty){
    initState();
    std::vector<MerciTransaction> txs;
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    MerciTransaction txEth = createMerciTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txs(1, txEth);
    auto result = executeBC(txs);
    
    std::vector<dev::Address> addrs = {createMerciAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasBase){
    initState();
    MerciTransaction txEth = createMerciTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    MerciTransaction txEth = createMerciTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasBase_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        MerciTransaction txEth = createMerciTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createMerciAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        MerciTransaction txEth = createMerciTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createMerciAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        MerciTransaction txEth = createMerciTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createMerciAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    MerciTransaction txEthCreate = createMerciTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    std::vector<dev::Address> addrs = {createMerciAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    MerciTransaction txEthCall = createMerciTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<MerciTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasBase_return_value){
    ...
    std::vector<MerciTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createMerciAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    MerciTransaction txEthCall = createMerciTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<MerciTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    MerciTransaction txEthCreate = createMerciTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createMerciAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    MerciTransaction txEthCall = createMerciTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<MerciTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    executeBC(txs);
    std::vector<MerciTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        MerciTransaction txEthCall = createMerciTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        MerciTransaction txEth = createMerciTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createMerciAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        MerciTransaction txEthCall = createMerciTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<MerciTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        MerciTransaction txEthCreate = createMerciTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        MerciTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createMerciTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createMerciAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createMerciAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        MerciTransaction txEthCall = createMerciTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    MerciTransaction txEthCreate = createMerciTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<MerciTransaction> txs(1, txEthCreate);
    executeBC(txs);
    std::vector<MerciTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createMerciAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        MerciTransaction txEthCall = createMerciTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
