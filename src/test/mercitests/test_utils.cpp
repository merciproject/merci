...
void initState(){
    ...
    const std::string dirMerci = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<MerciState>(new MerciState(dev::u256(0), MerciState::openDB(dirMerci, hashDB, dev::WithExisting::Trust), dirMerci + "/merciDB", dev::eth::BaseState::Empty));
...
MerciTransaction createMerciTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    MerciTransaction txEth;
    if(recipient == dev::Address()){
        txEth = MerciTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = MerciTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<MerciTransaction> txs){
