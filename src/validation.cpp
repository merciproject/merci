...
#if defined(NDEBUG)
# error "Merci cannot be compiled without assertions."
#endif
...
std::unique_ptr<MerciState> globalState;
...
const std::string strMessageMagic = "Merci Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            MerciDGP merciDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = merciDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = merciDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            MerciTxConverter converter(tx, NULL);
            ExtractMerciTX resultConverter;
            if(!converter.extractionMerciTransactions(resultConverter)){
            ...
            std::vector<MerciTransaction> merciTransactions = resultConverter.first;
            std::vector<EthTransactionParams> merciETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(MerciTransaction merciTransaction : merciTransactions){
                sumGas += merciTransaction.gas() * merciTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, merciTransaction.gasPrice());
                } else {
                    txMinGasPrice = merciTransaction.gasPrice();
                }
                VersionVM v = merciTransaction.getVersion();
                if(v.format!=0)
                ...
                if(merciTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(merciTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(merciTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += merciTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)merciTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(merciETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > merciTransactions.size())
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s MERCI additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 13 * COIN;
    // Subsidy is cut zero after 100 years
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // merci
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // merci
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    MerciTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<MerciTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path merciDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(merciDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(merciDir.string(), std::ios::in | std::ios::out);
...
bool MerciTxConverter::extractionMerciTransactions(ExtractMerciTX& mercitx){
    std::vector<MerciTransaction> resultTX;
    ...
    mercitx = std::make_pair(resultTX, resultETP);
    return true;
}

bool MerciTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool MerciTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
MerciTransaction MerciTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    MerciTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = MerciTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = MerciTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // merci
    MerciDGP merciDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setMerciSchedule(merciDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = merciDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = merciDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = merciDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // merci
...
///////////////////////////////////////////////////////////////////////////////////////// merci
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            MerciTxConverter convert(tx, &view, &block.vtx);

            ExtractMerciTX resultConvertMerciTX;
            if(!convert.extractionMerciTransactions(resultConvertMerciTX)){
            ...
            if(!CheckMinGasPrice(resultConvertMerciTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertMerciTX.first, blockGasLimit);
            ...
            for(MerciTransaction& qtx : resultConvertMerciTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertMerciTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertMerciTX.first[k].from(), resultConvertMerciTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(23100000000 + ((pindex->nHeight - 5000) * 13)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // merci
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // merci
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // merci
            globalState->setRootUTXO(oldHashUTXORoot); // merci
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // merci
        uint32_t sizeBlockDGP = merciDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Merci PoS we need to sync this after every block to ensure txdb is populated for
