...
bool ToHash160(const std::string& strMerciAddress, std::string& strHash160)
{
    CTxDestination merciAddress = DecodeDestination(strMerciAddress);
    if(!IsValidDestination(merciAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&merciAddress);
    if(keyid){
...
bool ToMerciAddress(const std::string& strHash160, std::string& strMerciAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination merciAddress = keyid;
    if(IsValidDestination(merciAddress)){
        strMerciAddress = EncodeDestination(merciAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToMerciAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToMerciAddress(tokenEvent.receiver, tokenEvent.receiver);
