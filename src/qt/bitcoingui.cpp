...
#include <qt/merciversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    merciVersionChecker = new MerciVersionChecker(this);

    if(fCheckForUpdates && merciVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(MERCI_RELEASES, MERCI_RELEASES);
        QString message(tr("New version of Merci wallet is available on the Merci source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Merci address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and merci: URIs)"));
    ...
    MRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/mrctoken"), tr("&MRC Tokens"), this);
    MRCTokenAction->setStatusTip(tr("MRC Tokens (send, receive or add Tokens in list)"));
    MRCTokenAction->setToolTip(MRCTokenAction->statusTip());
    MRCTokenAction->setCheckable(true);
    MRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(MRCTokenAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Merci addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Merci addresses"));
    ...
    openAction->setStatusTip(tr("Open a merci: URI or payment request"));//MERCI_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Merci command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(MRCTokenAction, tokenActions);
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    MRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Merci network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Merci"); // default title
