...
#define paternVersion "merci-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

MerciVersionChecker::MerciVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

MerciVersionChecker::~MerciVersionChecker()
{

}

bool MerciVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> MerciVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(MERCI_RELEASES)));
    QEventLoop event;
...
Version MerciVersionChecker::getMaxReleaseVersion()
{
