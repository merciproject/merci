    ...
    m_results(1)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in MERCI to send. Default = 0."));
    ui->labelSenderAddress->setToolTip(tr("The merci address that will be used as sender."));
...
void SendToContract::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: MERCI price per gas unit. Default = %1, Min = %2.").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
