...
bool WalletModel::isUnspentAddress(const std::string &merciAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    for (const COutput& out : vecOutputs)
    {
        ...
        if(fValidAddress && EncodeDestination(address) == merciAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
